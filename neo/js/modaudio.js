...
function AudioAddTrack(name,src,mimeType) {
	...
	if (this.tracks[i].mime == 'audio/it' || "audio/mod-compressed") {
		this.tracks[i].html = '<embed name="'+name+'" src="'+src+'" hidden="true" autostart="false">'
	}
	else {
...
function AudioGetMimeType(ext) {
	...
	else if (ext=='669' || ext=='ams' || ext=='AMS' || ext=='dbm' || ext=='DBM' || ext=='FAR' || ext=='far' || ext=='IT' || ext=='it' || ext=='mdl' || ext=='MDL' || ext=='mdr' || ext=='MDR' || ext=='MED' || ext=='med' || ext=='mod' || ext=='MOD' || ext=='mtm' || ext=='MTM' || ext=='nst' || ext=='NST' || ext=='okt' || ext=='OKT' || ext=='PTM' || ext=='ptm' || ext=='s3m' || ext=='S3M' || ext=='STM' || ext=='stm' || ext=='ult' || ext=='ULT' || ext=='wow' || ext=='WOW' || ext=='XM' || ext=='xm') return "audio/it"
	else if (ext=='mdz' || ext=='MDZ') return "audio/mod-compressed"
	else return ""
}
