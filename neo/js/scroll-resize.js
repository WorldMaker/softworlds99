// Scroll Object [resizable]
// a resizable version of the Scrollv1
// 19990505
...
//
//	thanks to flavio curti {fcu@futurecom.ch
...
function ScrollBuild() {
	...
	css(nm+'ArrowU',this.bar.x,this.arrow.uy,bw,((this.clipImages)?this.arrow.uh:null),this.arrow.color,'hidden')+
	css(nm+'ArrowD',this.bar.x,this.arrow.dy,bw,((this.clipImages)?this.arrow.dh:null),this.arrow.color,'hidden')+
	css(nm+'ArrowUC',this.bar.x,this.arrow.uy,bw,this.arrow.uh)+
	css(nm+'ArrowDC',this.bar.x,this.arrow.dy,bw,this.arrow.dh)+
	css(nm+'Bar',this.bar.x,this.bar.y,bw,((this.clipImages)?this.bar.h:null),this.bar.color,'hidden',null,this.bar.tile)+
	...
	if (this.bar.images) this.divEnd += '<IMG SRC="'+this.bar.image.src+'" WIDTH="'+this.bar.imageW+'" HEIGHT="100%" name="barimage">\n'
...
function ScrollActivate(w,h) {
	...
	//
	//added by fcu for resizing, i need access to the css-props of these layers tooo...
	//
	this.barlyrt = new DynLayer(this.name+'Bar',this.name)
	this.barlyrt.h=this.h-(2*this.arrow.dh);
	this.barlyrt.css.height=this.barlyrt.h;
	this.barlyrt.css.clip.bottom=this.barlyrt.h;this.barlyrt.show();
	this.textlyrt = new DynLayer(this.name+'TextC',this.name)
	this.textlyrt.clipInit(0,this.w,this.h,0);
	this.scrollbglyr = new DynLayer(this.name+'BG',this.name)
	this.arrowur.show();	
	this.arrowdr.show();	
}
...
//
//this is it... resizing, real ugly, but works nice...
//
function ScrollResize(width,height){
		this.h=height;
		this.w=width;
		
//		alert(this.x+"*"+this.y+"*"+this.h+"*"+this.w);
//		if (width<800){nwidth="800";}else{nwidth=width}
//		if (height<600){nwidth="600";}else{nheight=height}
		nbarheight=height-(this.arrow.uh+this.arrow.dh);
		nboxheight=height-(this.arrow.uh);
		this.arrowur.moveTo(width,null);
		this.arrowu.moveTo(width,null);
		this.arrowdr.moveTo(width,height-this.arrow.dh);
		this.arrowd.moveTo(width,height-this.arrow.dh);

		this.barlyr.moveTo(width,null);
		this.barlyr.css.height=nbarheight;
		this.barlyr.h=nbarheight;
		this.barlyr.css.clip.bottom=nbarheight;
		
		this.barlyrt.moveTo(width,null);
		this.barlyrt.h=nbarheight;
		this.barlyrt.css.height=nbarheight;
		this.barlyrt.css.clip.bottom=nbarheight;
		
		
		this.bar.h=nbarheight;
//		this.barlyrt.doc.images['barimage'].height=nbarheight;

		this.textlyr.w=width;
		this.textlyr.h=height;
		this.textlyr.css.width=width;
		this.textlyr.css.height=height;		
		this.textlyr.css.clip.right=width;
		this.textlyr.css.clip.bottom=height;
		
		this.lyr.w=width+this.bar.w;;
		this.lyr.h=height;
		this.lyr.css.width=width+this.bar.w;;
		this.lyr.css.height=height;		
		this.lyr.css.clip.right=width+this.bar.w;;
		this.lyr.css.clip.bottom=height;

		this.textlyrt.w=width;
		this.textlyrt.h=height;
		this.textlyrt.css.width=width;
		this.textlyrt.css.height=height;		
		this.textlyrt.css.clip.right=width;
		this.textlyrt.css.clip.bottom=height;
		this.textlyrt.clipTo(0,width,height,0);
//		alert(layerProps(this.textlyrt));
		this.scrollbglyr.w=width;
		this.scrollbglyr.h=height;
		this.scrollbglyr.css.width=width;
		this.scrollbglyr.css.height=height;		
		this.scrollbglyr.css.clip.right=width;
		this.scrollbglyr.css.clip.bottom=height;
//		alert(layerProps(this.textlyrt));
		this.activate();		
}
function layerProps(layer){
		bla="name: "+layer.id+"\ntop:"+layer.css.top+" left:"+layer.css.left+" width:"+layer.w+" height:"+layer.h+"\nclip left:"+layer.css.clip.left+" right:"+layer.css.clip.right+" bottom:"+layer.css.clip.bottom+" top:"+layer.css.clip.top+" vis:"+layer.css.visibility;
		return bla;
}	
Scroll.count = 0
